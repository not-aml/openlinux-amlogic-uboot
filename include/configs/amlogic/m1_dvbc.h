...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"mmcargs=setenv bootargs console=${console} " \
		"root=254:2 rw " \
		"rootfstype=ext3 rootwait init=/init\0" \
	"nandargs=setenv bootargs console=${console} " \
		"root=/dev/mtdblock4 rw " \
		"rootfstype=yaffs2 init=/init\0" \
	"loaduimage=fatload mmc 0 ${loadaddr} uImage\0" \
	"mmcboot=echo Booting from mmc ...; " \
		"mmcinfo; " \
		"run mmcargs; " \
		"mmcinfo 0; " \
		"fatload mmc 0 ${loadaddr} uImage.android; " \
		"bootm ${loadaddr}\0" \
	...
	"chipname=8726m\0" \
	"machid=2957\0" \
	"bootargs=root=253:2 rw rootfstype=ext3 init=/init console=ttyS0,115200\0"
