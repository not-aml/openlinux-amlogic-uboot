...
static ulong mmc_bwrite(int dev_num, ulong start, lbaint_t blkcnt, const void*src)
{
	...
	int err, write_cnt = 0, total_cnt = 0, write_start;
	int stoperr = 0;
    ...
    do {
	    ...
	    if ((blkcnt - total_cnt) > 1)
		    cmd.cmdidx = MMC_CMD_WRITE_MULTIPLE_BLOCK;
	    else
		    cmd.cmdidx = MMC_CMD_WRITE_SINGLE_BLOCK;

        if ((blkcnt - total_cnt) >= 256)
            write_cnt = 256;
        else
            write_cnt = (blkcnt - total_cnt);

	    if (mmc->high_capacity)
		    cmd.cmdarg = write_start + total_cnt;
	    else
		    cmd.cmdarg = (write_start + total_cnt) * blklen;

	    cmd.resp_type = MMC_RSP_R1;
	    cmd.flags = 0;

	    data.src = src + total_cnt * blklen;
	    data.blocks = write_cnt;
...
{
	int err, read_cnt = 0, total_cnt = 0, read_start, i;
	int blklen;
	int stoperr = 0;
	struct mmc_data data;
	struct mmc_cmd cmd;
    ...
    do {
        ...
        if ((blkcnt - total_cnt) >= 256)
            ...
            read_cnt = (blkcnt - total_cnt);

	    if (mmc->high_capacity)
		    cmd.cmdarg = read_start + total_cnt;
	    else
		    cmd.cmdarg = (read_start + total_cnt) * blklen;

	    cmd.resp_type = MMC_RSP_R1;
	    cmd.flags = 0;

	    data.dest = dst + total_cnt * blklen;
	...
	for (i = start; i < start + blkcnt; i++, dst += mmc->read_bl_len) {
		...
		if (err) {
			printf("single block read failed: %d\n", err);
			return i - start;
		}
	}*/
...
sd_send_op_cond(struct mmc *mmc)
{
	...
	do {
		...
		udelay(1000);
	} while ((!(((uint *)cmd.response)[0] & OCR_BUSY) || err) && timeout--);
...
int mmc_send_op_cond(struct mmc *mmc)
{
	...
	do {
		...
		cmd.cmdarg =  0x40FF8000;//mmc->voltages;
		cmd.flags = 0;
...
#ifndef CONFIG_MMC_FORCE_1BIT_MODE
...
int sd_switch(struct mmc *mmc, int mode, int group, u8 value, u8 *resp)
{
	...
	data.flags = MMC_DATA_READ;
       ...
       {
            for(i = 16; i >=0; i--)
                ((u32 *)resp)[i] = __be32_to_cpu(((u32 *)resp)[i]);
       }
       return ret;
}
