...
int aml_sdio_io_init(struct mmc *mmc, ulong flag)
{
	...
	AML_CARD_SD_INFO_t *aml_priv = NULL;
	if(mmc == NULL)
		return -1;
...
void aml_sd_cfg_swth(struct mmc *mmc)
{
	...
	AML_CARD_SD_INFO_t *aml_priv = mmc->priv;
	...
	unsigned clk=get_clk81();
	unsigned clk_div=(get_clk81() / (2*mmc->clock));
...
{
	int level;
	AML_CARD_SD_INFO_t *sd_inand_info = mmc->priv;
