...
//	ops.len=w_size;
	...
	for(area_loop=0;area_loop<1;area_loop++)
	{
		ops.datbuf=rom_inter_buff;
		ret=mtd->write_oob(mtd,area_loop*area_size,&ops);						//that is a recursion , need 384 , but write 1536 , use bch 60 no ran 	
		if(ret)
		{
			err_flag=1;
		...
		for(cur=0,i=1;cur<(len);i++)
		{
			if((cur+w_size)>len)
			{
				memset(rom_inter_buff,0xff,mtd->writesize);
				memcpy((char*)rom_inter_buff, (char*)&p[cur],len-cur);
				ops.datbuf=rom_inter_buff;
			}
			else
			...
			while(1){
				if (nand_block_isbad (mtd, (w_offset+i*(mtd->writesize)) & ~(mtd->erasesize - 1))) {
					printf ("Skip bad block 0x%08llx\n",
						(w_offset+i*(mtd->writesize)) & ~(mtd->erasesize - 1));
					w_offset += nand->erasesize;
				}
				else{
					break;	
				}
			...
			ret=mtd->write_oob(mtd,area_loop*area_size+i*(mtd->writesize)+w_offset,&ops);
			if(ret)
			{
				err_flag=1;
				printk("mtd->writeoob err at  %lld \n",area_loop*area_size+i*(mtd->writesize));	
			}
			...
			u_char *buf)
{
	...
	{
		while(1){
			if(!((r_offset+i*(mtd->writesize))% mtd->erasesize))
			{
				if (nand_block_isbad (mtd, (r_offset+i*(mtd->writesize)) & ~(mtd->erasesize - 1))) {
					printf ("Skip bad block 0x%08llx\n",
						(r_offset+i*(mtd->writesize)) & ~(mtd->erasesize - 1));
					r_offset += nand->erasesize;
				}
				else{
					break;	
				}
		...
		ret=mtd->read(mtd,i*(mtd->writesize)+r_offset,mtd->writesize,plen,rom_inter_buff);
		if(ret)
		{
			err_flag=1;
			printk("mtd->read err at  %lld \n",i*(mtd->writesize)+r_offset);	
		}
		memcpy(&buf[cur],rom_inter_buff,w_size);
		cur	+=w_size;
		if(cur==0x00007E00)
		{
			memset(&buf[cur],0x00,0x200);
			cur=32768;
		}
	}	
...
{
	int error, test_num = 0, i, blk_num;
	...
	for (i=pagesize * 1024/blocksize; i<blk_num; i++) {
		addr = blocksize;
		addr *= i;
