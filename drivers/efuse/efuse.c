...
int efuse_chk_written(int usr_type)
{
	...
	switch(usr_type){
		...
		case USR_HDMHDCP: ppos =12; count=312;op=re_hdcp;break;
...
unsigned char *efuse_read_usr(int usr_type)
{
	...
	switch(usr_type){
		...
		case USR_HDMHDCP: ppos =12; count=312;dec_len=310;op=re_hdcp;break;
...
int efuse_write_usr(int usr_type, unsigned char *data)
{
	...
	switch(usr_type){
		...
		case USR_HDMHDCP: ppos =12; 	count=312;	enc_len=310;data_len=300;	ip=re_hdcp;break;
		case USR_USERIDF: ppos =320; 	count=64;	enc_len=62;	data_len=60;	ip=re_usid;break;
	}
