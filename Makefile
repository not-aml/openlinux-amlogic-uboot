...
ROMBOOT_SPL  = romboot_spl
U_BOOT_AML   =$(OBJTREE)/u-boot-aml.bin
HW_DEBUG_ROM=hw_debug_rom
AML_DEBUG_ROM=$(OBJTREE)/aml-debug-rom.bin
AML_DEBUG_ROM_SD=$(OBJTREE)/aml-debug-rom-sd.bin
endif
ifeq ($(CONFIG_UCL),y)
U_BOOT_AML_UCL	=$(OBJTREE)/u-boot-aml-ucl.bin
endif
...
ALL += $(obj)u-boot.srec $(obj)u-boot.bin $(obj)System.map $(U_BOOT_NAND) \
	$(U_BOOT_ONENAND) $(U_BOOT_AML) $(AML_DEBUG_ROM_SD) $(U_BOOT_AML_UCL)
...
$(obj)u-boot-ucl: $(obj)piggy.ucl $(UCL_BOOTLIBS)
		cd $(LNDIR) && $(LD) -Bstatic -T $(TOPDIR)/arch/$(ARCH)/cpu/$(CPU)/uclboot/u-boot.lds \
			-Ttext $(UCL_TEXT_BASE) $(PLATFORM_LDFLAGS) --cref arch/$(ARCH)/cpu/$(CPU)/uclboot/start.o \
			--start-group $(__UCLBOOTLIBS) $(CPUDIR)/$(SOC)/lib$(SOC).a lib/ucl/libucl.a --end-group $(PLATFORM_LIBS) \
			-Map u-boot-ucl.map -o u-boot-ucl
...
$(HW_DEBUG_ROM) : $(AML_DEBUG_ROM_SD)
	
$(AML_DEBUG_ROM_SD): $(AML_DEBUG_ROM) tools 
		$(obj)tools/convert $< $@  -cpu M1
$(U_BOOT_AML): $(ROMBOOT_SPL) $(obj)u-boot.bin
		$(obj)tools/convert $(obj)u-boot-spl.bin $(obj)u-boot.bin $@ -cpu M1

$(U_BOOT_AML_UCL): $(ROMBOOT_SPL) $(obj)u-boot-ucl.bin
		$(obj)tools/convert $(obj)u-boot-spl.bin $(obj)u-boot-ucl.bin $@ -cpu M1

$(ROMBOOT_SPL): $(TIMESTAMP_FILE) $(VERSION_FILE) $(obj)include/autoconf.mk
		$(MAKE) -C $@/$(ARCH)/$(CPU)/$(SOC) all
$(AML_DEBUG_ROM):$(TIMESTAMP_FILE) $(VERSION_FILE) $(obj)include/autoconf.mk
	$(MAKE) -C hw_debug_rom/$(ARCH)/$(CPU)/$(SOC) all
aml_debug_rom:$(HW_DEBUG_ROM)
...
m1_dvbc_m1_config: unconfig
	$(MKCONFIG) $(@:_config=) arm aml_meson dvbc_8226_m1 amlogic m1
m1_8726m_mid_config: unconfig
	$(MKCONFIG) $(@:_config=)  arm aml_meson aml8726m_mid amlogic m1
m1_8726m_arm_config: unconfig
	$(MKCONFIG) $(@:_config=)  arm aml_meson aml8726m_arm amlogic m1
