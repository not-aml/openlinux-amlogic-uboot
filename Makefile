...
export	ARCH CPU BOARD VENDOR SOC HARDWARE_MODE
...
LIBS += drivers/input/touchscreen/libtouchscreen.a
...
LIBS += drivers/efuse/efuse.a
LIBS += drivers/extern_io/extern_io.a
...
UCL_BOOTLIBS += arch/$(ARCH)/cpu/$(CPU)/uclboot/ucl_lib_$(CPU).a
...
ifeq ($(CONFIG_AMLROM_U_BOOT),y)
FIRMWARE		= $(OBJTREE)/firmware.bin
ifeq ($(CONFIG_UCL),y)
U_BOOT_AML	=$(OBJTREE)/u-boot-aml-ucl.bin
else
U_BOOT_AML   =$(OBJTREE)/u-boot-aml.bin
endif
endif
...
ALL += $(obj)u-boot.srec $(obj)u-boot.bin $(obj)System.map $(U_BOOT_NAND) \
	$(U_BOOT_ONENAND) $(U_BOOT_AML) $(FIRMWARE) $(U_BOOT_AML_UCL)
...
$(obj)u-boot-ucl.bin:	$(obj)u-boot-ucl
		$(OBJCOPY) ${OBJCFLAGS} -O binary $< $@
		mv $< $(obj)u-boot-ucl.out
...
$(obj)u-boot-ucl: $(obj)piggy.ucl $(UCL_BOOTLIBS) $(LIBS)  
		$(LD) -Bstatic -T $(TOPDIR)/arch/$(ARCH)/cpu/$(CPU)/uclboot/u-boot.lds \
			...
			--start-group $(UCL_BOOTLIBS) $(obj)$(CPUDIR)/$(SOC)/lib$(SOC).a $(obj)lib/ucl/libucl.a   \
			--end-group  $(PLATFORM_LIBS) \
			-Map $(obj)u-boot-ucl_a.map -o $@
	
$(obj)piggy.ucl: $(obj)u-boot.bin		
		$(obj)/tools/uclpack $< $@
...
$(UCL_BOOTLIBS):	$(obj)piggy.ucl depend $(SUBDIRS)
		$(MAKE) -C $(dir $(subst $(obj),,$@))
...
firmware   : $(FIRMWARE)
ifeq ($(CONFIG_AML_MESION_3),y)
PLUS_BINARY=$(shell echo $(CONFIG_PLUS_BINARY))
$(obj)u-boot-aml-ucl.bin: $(PLUS_BINARY) $(FIRMWARE) $(obj)piggy.ucl
	$(obj)tools/convert --soc $(SOC)  -s $(FIRMWARE) -i $(obj)piggy.ucl -o $(@:%=%.tmp)
ifneq ($(PLUS_BINARY),)	
	$(obj)tools/convert --soc $(SOC)  -s $(@:%=%.tmp) -i $(PLUS_BINARY) -a $(CONFIG_ARC_LOAD_ADDR) -o $@
else
	mv	$(@:%=%.tmp) $@
endif	

$(FIRMWARE): $(TIMESTAMP_FILE) $(VERSION_FILE) tools $(obj)include/autoconf.mk $(LIBS) 
	$(MAKE) -C $(basename $(@F))/$(ARCH)/$(CPU)/$(SOC) all FIRMWARE=$@ UCL_BOOTLIBS=$(obj)lib/ucl/libucl.a
	mv $(obj)aml-debug-rom.bin $(obj)aml-debug-rom.old.bin
	$(obj)tools/convert --soc $(SOC)  -s  $(obj)aml-debug-rom.old.bin -o $(obj)aml-debug-rom.bin
	$(obj)tools/convert --soc $(SOC)  -s $(obj)firmware.bin  -o $(obj)tmp.bin
	mv $@ $(@:%.bin=%.old.bin)
	mv $(obj)tmp.bin $@
	
		
else
$(FIRMWARE): $(TIMESTAMP_FILE) $(VERSION_FILE) tools $(obj)include/autoconf.mk
	$(MAKE) -C $(basename $(@F))/$(ARCH)/$(CPU)/$(SOC) all FIRMWARE=$@ 
	mv $(obj)aml-debug-rom.bin $(obj)aml-debug-rom.old.bin
	$(obj)tools/convert --soc $(SOC)  -s  $(obj)aml-debug-rom.old.bin -o $(obj)aml-debug-rom.bin
	$(obj)tools/convert --soc $(SOC)  -s $(obj)firmware.bin  -o $(obj)tmp.bin
	mv $@ $(@:%.bin=%.old.bin)
...
$(obj)u-boot-aml-ucl.bin: $(FIRMWARE) $(obj)u-boot-ucl.bin
	$(obj)tools/convert --soc $(SOC)  -s $(FIRMWARE) -i $(obj)u-boot-ucl.bin -o $@
endif

$(obj)u-boot-aml.bin: $(FIRMWARE) $(obj)u-boot.bin
	mv $(obj)aml-debug-rom.bin $(obj)aml-debug-rom.old.bin
	$(obj)tools/convert --soc $(SOC)  -s  $(obj)aml-debug-rom.old.bin -o $(obj)aml-debug-rom.bin
	$(obj)tools/convert --soc $(SOC)  -s $(FIRMWARE) -i $(obj)u-boot.bin -o $@
...
clean:
	...
	@find $(OBJTREE) -type f \
		...
		-o -name '*.o'	-o -name '*.a' -o -name '*.exe'	-o -name '*.d' \) -print \
		| xargs rm -f
