...
int saveenv(void)
{
	...
	puts ("Writing to Nand... \n");
	printk("env offset: %x\n", offset);
	if (writeenv(offset, (u_char *) env_ptr)) {
		puts("FAILED!\n");
		return 1;
	}
	//puts("Successful!\n");
	nand_erase_options.offset = offset + last_valid_block*blocksize;
	if (nand_erase_opts(mtd, &nand_erase_options)){
...
int readenv (size_t offset, u_char * buf)
{
	...
	if(map_flag == 0) {
		for(i=0; i<CONFIG_ENV_BLOCK_NUM; i++){  //scan block, find valid env block
			...
			nand_read(mtd, addr, &len, temp_buf);
			env_map[i].env_valid = ((temp_buf[0] == 0xaa)&&(temp_buf[1] == 0x55));
			env_flag += env_map[i].env_valid;
			_debug("read %d: %x, %x, %x %x\n",i,temp_buf[0],temp_buf[1],env_map[i].block_valid, addr);
		}
		if(env_flag == 0){
			for(i=0;i<CONFIG_ENV_BLOCK_NUM;i++){
				nand_erase_options.offset = offset + i*blocksize;
				env_map[i].block_valid = (nand_erase_opts(mtd, &nand_erase_options)==0);
		...
		if(env_flag > 1){ //conflict resolve, if more than one block has valid env flag
			if((env_map[0].env_valid==1) && (env_map[CONFIG_ENV_BLOCK_NUM-1].env_valid==1)){
				...
				env_map[CONFIG_ENV_BLOCK_NUM-1].block_valid=(nand_erase_opts(mtd, &nand_erase_options)==0);
			}
			else{
