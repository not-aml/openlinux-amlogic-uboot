...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"preboot=run detect_storage;get_rebootmode; clear_rebootmode; echo reboot_mode=${reboot_mode}; run factoryreset_wipe_data; if test ${reboot_mode} = usb_burning; then tiny_usbtool 20000; fi; run nand_key_burning; run upgrade_check; run updatekey_or_not; run irremote_update; run switch_bootmode\0" \
	"mbr_write=if test ${upgrade_step} != 2; then mmcinfo 1; mmc read 1 82000000 0 1; mw.l 820001fc 0 1; mmc write 1 82000000 0 1;fi;\0" \
	"upgrade_check=if itest ${upgrade_step} == 1; then defenv_without reboot_mode;setenv upgrade_step 2; save; fi; if itest ${upgrade_step} == 0; then defenv; save; run update; fi; if itest ${upgrade_step} == 3; then defenv; save; run update; fi;\0" \
	...
	"nandargs=run cvbscheck; nand read logo 0x83000000 0 800000;unpackimg 0x83000000; cp ${bootup_offset} 0x85100000 ${bootup_size}; setenv bootargs root=/dev/system rw rootfstype=ext4 rootwait init=/init console=ttyS0,115200n8 logo=osd1,0x85100000,${outputtemp},full androidboot.resolution=${outputmode} hdmimode=${hdmimode} cvbsmode=${cvbsmode} hlt vmalloc=128m mem=256m a9_clk_max=1512000000 vdachwswitch=${vdacswitchmode} hdmitx=${cecconfig}\0"\
	"mmcargs=run cvbscheck; mmcinfo 1; mmc read 1 0x83000000 ${mmc_logo_offset} ${mmc_lk_size};unpackimg 0x83000000; cp ${bootup_offset} 0x85100000 ${bootup_size}; setenv bootargs root=/dev/cardblksd2 rw rootfstype=ext3 rootwait init=/init console=ttyS0,115200n8 logo=osd1,0x85100000,${outputtemp},full androidboot.resolution=${outputmode} nohlt vmalloc=128m mem=256m a9_clk_max=1512000000 vdachwswitch=${vdacswitchmode} hdmitx=${cecconfig}\0"\
	...
	"bootargs=root=/dev/cardblksd2 rw rootfstype=ext3 rootwait init=/init console=ttyS0,115200n8 nohlt vmalloc=128m mem=256m logo=osd1,0x85100000,1080p\0" \
	"storage=null\0" \
  ...
  "bootsdcard=mmcinfo; ext2load mmc 0 82000000 boot.img; setenv bootargs root=/dev/mmcblk0p1 rw rootfstype=ext2 rootwait init=/init console=ttyS0,115200n8 logo=osd1,0x85100000,1080p,full hdmimode=1080p cvbsmode=576cvbs hlt vmalloc=128m mem=256m a9_clk_max=1512000000 vdachwswitch=cvbs hdmitx= firmware=${firmware}; bootm\0" \
  "bootupdate=mmcinfo; fatload mmc 0 ${loadaddr} boot.img; setenv firmware rootfs.tar.gz; setenv bootargs root=/dev/mmcblk0p1 rw rootfstype=ext2 rootwait init=/init console=ttyS0,115200n8 logo=osd1,0x85100000,1080p,full hdmimode=1080p cvbsmode=576cvbs hlt vmalloc=128m mem=256m a9_clk_max=1512000000 vdachwswitch=cvbs hdmitx= firmware=${firmware}; bootm\0" \
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"nand_key_burning=saradc open 4;if saradc get_in_range 0x164 0x1b4 ;then msleep 500;if saradc get_in_range 0x164 0x1b4; then tiny_usbtool 20000; fi; fi\0" \
	"cvbscheck=setenv outputtemp ${outputmode};if test ${outputmode} = 480i; then if test ${cvbsenable} = true; then setenv outputtemp 480cvbs;fi;fi; if test ${outputmode} = 576i; then if test ${cvbsenable} = true; then setenv outputtemp 576cvbs;fi;fi\0" \
	"nandargs=run cvbscheck; nand read logo 0x83000000 0 300000;unpackimg 0x83000000; cp ${bootup_offset} 0x85100000 ${bootup_size}; setenv bootargs root=/dev/cardblksd2 rw rootfstype=ext3 rootwait init=/init console=ttyS0,115200n8 logo=osd1,0x85100000,${outputtemp},full androidboot.resolution=${outputmode} nohlt vmalloc=128m mem=256m a9_clk_max=1512000000 vdachwswitch=${vdacswitchmode} hdmitx=${cecconfig} quiet lpj=lpj=11935744\0"\
	"switch_bootmode=if test ${reboot_mode} = factory_reset; then run recovery;else if test ${reboot_mode} = update; then run recovery;fi;fi\0" \
	"nandboot=echo Booting from nand ...;run nandargs;nand read boot ${loadaddr} 0 600000;hdcp prefetch nand;bootm\0" \
...
//note: please DO NOT remove following check code
#if !defined(M6_DDR3_1GB) && !defined(M6_DDR3_512M) && !defined(M6_DDR3_256M)
