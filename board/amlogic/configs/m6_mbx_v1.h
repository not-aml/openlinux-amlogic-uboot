...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"nandargs=run cvbscheck; nand read logo 0x83000000 0 800000;unpackimg 0x83000000; cp ${bootup_offset} 0x85100000 ${bootup_size}; setenv bootargs root=/dev/system rw rootfstype=ext4 rootwait init=/init console=ttyS0,115200n8 logo=osd1,0x85100000,${outputtemp},full androidboot.resolution=${outputmode} hdmimode=${hdmimode} cvbsmode=${cvbsmode} hlt vmalloc=256m mem=1024m a9_clk_max=1512000000 vdachwswitch=${vdacswitchmode} hdmitx=${cecconfig}\0"\
	"mmcargs=run cvbscheck; mmcinfo 1; mmc read 1 0x83000000 ${mmc_logo_offset} ${mmc_lk_size};unpackimg 0x83000000; cp ${bootup_offset} 0x85100000 ${bootup_size}; setenv bootargs root=/dev/cardblksd2 rw rootfstype=ext3 rootwait init=/init console=ttyS0,115200n8 logo=osd1,0x85100000,${outputtemp},full androidboot.resolution=${outputmode} nohlt vmalloc=256m mem=1024m a9_clk_max=1512000000 vdachwswitch=${vdacswitchmode} hdmitx=${cecconfig}\0"\
  ...
  "bootsdcard=mmcinfo; ext2load mmc 0 82000000 boot.img; setenv bootargs root=/dev/mmcblk0p1 rw rootfstype=ext2 rootwait init=/init console=ttyS0,115200n8 logo=osd1,0x85100000,1080p,full hdmimode=1080p cvbsmode=576cvbs hlt vmalloc=256m mem=1024m a9_clk_max=1512000000 vdachwswitch=cvbs hdmitx= ; bootm\0" \
  "bootupdate=mmcinfo; fatload mmc 0 ${loadaddr} boot.img; setenv firmware rootfs.tar.gz; setenv bootargs root=/dev/mmcblk0p1 rw rootfstype=vfat rootwait init=/init console=ttyS0,115200n8 logo=osd1,0x85100000,1080p,full hdmimode=1080p cvbsmode=576cvbs hlt vmalloc=256m mem=1024m a9_clk_max=1512000000 vdachwswitch=cvbs hdmitx= firmware=${firmware}; bootm\0" \
