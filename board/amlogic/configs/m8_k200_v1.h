...
#define CONFIG_EXTRA_ENV_SETTINGS \
    ...
    \
   	"storeargs="\
        "setenv bootargs ${bootargs} logo=osd1,loaded,${fb_addr},${outputmode},full hdmimode=${hdmimode} cvbsmode=${cvbsmode} androidboot.firstboot=${firstboot} hdmitx=${cecconfig} root=/dev/system rw rootfstype=ext4\0"\
    \
	"switch_bootmode="\
    ...
    "bootsdcard="\
        "if mmcinfo; then "\
           "if ext2load mmc 0 ${loadaddr} boot.img; then "\
               "setenv bootargs console=ttyS0,115200n8 no_console_suspend storage=4 "\
               "logo=osd1,loaded,${fb_addr},${outputmode},full hdmimode=${hdmimode} cvbsmode=${cvbsmode} hdmitx= "\
               "root=/dev/mmcblk0p1 rw rootfstype=ext2 init=/init rootwait; bootm; "\
           "fi; "\
        "fi\0" \
    \
    "bootupdate="\
        "if mmcinfo; then "\
            "if fatload mmc 0 ${loadaddr} boot.img; then "\
                "setenv firmware rootfs.tar.gz; "\
                "setenv bootargs console=ttyS0,115200n8 no_console_suspend storage=4 "\
                "logo=osd1,loaded,${fb_addr},${outputmode},full hdmimode=${hdmimode} cvbsmode=${cvbsmode} hdmitx= "\
                "root=/dev/mmcblk0p1 rw rootfstype=vfat init=/init rootwait firmware=${firmware}; bootm; "\
            "fi; "\
        "fi\0" 
