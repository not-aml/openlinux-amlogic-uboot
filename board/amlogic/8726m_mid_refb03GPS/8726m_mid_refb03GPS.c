...
static int  sdio_detect(unsigned port)
{
	...
	//return 0;//(readl(P_PREG_CGPIO_I)&(1<<5))?1:0;
		return (readl(P_PREG_CGPIO_I)&(1<<0))?1:0;//0x2011 bit 0
}
...
{
	CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_11, (1<<0));
	CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_0, (1<<15));
	CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_1, (1<<4));
	CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_9, (1<<16));
	CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_8, (1<<5));
	CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_12, (1<<13));
...
static void Set_Lcd_On(void)
{
	...
	CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_8, (1<<5));
	CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_12, (1<<13));
	set_gpio_val(GPIOA_bank_bit(1), GPIOA_bit_bit0_14(1), 1);
	set_gpio_mode(GPIOA_bank_bit(1), GPIOA_bit_bit0_14(1), GPIO_OUTPUT_MODE);
}
            ...
            default:
            {
                ...
                else
                {
                    ...
                    if(get_powerkey_hold_count())
                    {
                    	  ...
                    	  //logo_display();
#ifdef CONFIG_BATTERY_CHARGING
                        if(get_battery_percentage() < 15) //when power < 20%, power down.
                        {
                            power_low_display();
                            sdelay(2);
                            power_unhold();
...
#ifdef CONFIG_EXIO_SN7325
	printk( "amlogic camera 0308 driver: init CONFIG_SN7325. \n");  
    sn7325_set_io_dir(sn7325_get_io_dir() & ~PP0);
    sn7325_set_output_level(sn7325_get_output_level() | PP0);

	msleep(20);
	
	sn7325_set_io_dir(sn7325_get_io_dir() & ~PP4);
	sn7325_set_output_level(sn7325_get_output_level() & ~PP4);
...
#ifdef CONFIG_EXIO_SN7325
	printk( "amlogic camera 2005 driver: init CONFIG_SN7325. \n");  
    sn7325_set_io_dir(sn7325_get_io_dir() & ~PP0);
    sn7325_set_output_level(sn7325_get_output_level() | PP0);
...
static void cs42l52_pwr_reset(void)
{
    //reset
   CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_5, (1<<31));
   CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_6, (1<<9));
...
static void fotaltech_ts_power_on(void)
{
...
static struct i2c_board_info aml_i2c_info[] = {
    ...
    {
        I2C_BOARD_INFO("smartbattery", 0x55),
        .device_init = NULL,
    },
#endif
#ifdef CONFIG_SND_AML_M1_MID_CS42L52
    {
        ...
        .device_init = cs42l52_pwr_reset,
    },
#endif
#ifdef CONFIG_FOCALTECH_CAPACITIVE_TOUCHSCREEN
    {
        I2C_BOARD_INFO("ft5x06", 0x38),
        .device_init = fotaltech_ts_power_on,
    },
#endif
};
#endif
