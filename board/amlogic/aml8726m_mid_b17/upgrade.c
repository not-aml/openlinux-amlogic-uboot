            ...
            if(!run_command (str, 0))
            {
                ...
                //if(size != size1)
                {
                    ...
                    while(retry-- > 0)
                    {
                        ...
                        run_command ("sf erase ${bootloader_start} ${bootloader_size}", 0);
                        run_command ("sf write ${loadaddr} ${bootloader_start} ${bootloader_size}", 0);
...
void into_recovery(void)
{
    ...
    run_command ("mmc read 1 84100000 ${logo_start} ${logo_size}", 0);
    if(!run_command ("mmcinfo 0", 0))
    {
        for(i = 0; i < SCAN_MMC_PARTITION; i++)
        {
            sprintf(str, "fatexist mmc0 0:%d ${recovery_path}", (i + 1));
            if(!run_command (str, 0))
            {
                UPGRADE_DPRINT("recovery in SD Card!!!\n");
                sprintf(str, "fatload mmc0 0:%d ${loadaddr} ${recovery_path}", (i + 1));
                run_command (str, 0);
                run_command ("bootm", 0);
            }
        }
...
recovery_in_nand:
    UPGRADE_DPRINT("recovery in inand!!!\n");
