...
static void init_pctl(struct ddr_set * ddr_setting)
{
    ...
    APB_Wr(PCTL_IOCR_ADDR, (APB_Rd(PCTL_IOCR_ADDR) | 0xfe000000));
...
//                    (0x10<<8)|(1<<17)
    ...
    APB_Wr(PCTL_PHYCR_ADDR, APB_Rd(PCTL_PHYCR_ADDR)&(~0x108));
...
static inline void dtu_wait_ready(void)
{
...
next:
    ...
    for(i=0;i<=DDR_MAX_WIN_LEN -DDR_RDGR_LEN;i++)
    {
        if(*(p+i) + *(p+i+1) + *(p+i+2) + *(p+i+3) == 64)
        {
            if(!i)
                *best = 2;
            else if(i == DDR_MAX_WIN_LEN - 4)
                *best = DDR_MAX_WIN_LEN - 3;
            else
            {
                if(*(p+i-1)>*(p+i+4))
                    *best = i + 1;
...
STATIC_PREFIX unsigned ddr_init_test(void)
{
    ...
    serial_putc('\n');
#ifdef CONFIG_ENABLE_MEM_DEVICE_TEST    
    for(i=0;i<MEM_DEVICE_TEST_ITEMS_BASE+ddr_start_again&&por_cfg==0;i++)
#else    
