...
void pll_clk_test(void) 
{
    ...
    unsigned long  i,j;
    serial_puts("Start pll test: ");
	for(i=0;i<21;i++)
	{

		if(i==2) continue;
		serial_put_num((int)(i));
		clk_freq = clk_util_clk_msr(i,      // unsigned long   clk_mux,             // Measure A9 clock
								);   // unsigned long   uS_gate_time )       // 50us gate time
		serial_puts("clk is: ");
		serial_put_num((int)(clk_freq>>6));
		serial_puts(".");
		serial_put_num((int)(clk_freq*100)>>6);
		serial_puts(" MHZ\n");
...
//		serial_put_num((int)(i));
		...
		serial_put_num((int)(clk_freq>>6));
		serial_puts(".");
		serial_put_num((int)((clk_freq&0x3f)*100)>>6);
		serial_puts(" MHZ\n");
	}
}
