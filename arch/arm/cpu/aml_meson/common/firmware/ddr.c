...
{
    return (unsigned)memTestDevice((datum *)PHYS_MEMORY_START
		    ,PHYS_MEMORY_SIZE);
}
static inline unsigned lowlevel_mem_test_data(void)
{
    return (unsigned)memTestDataBus((datum *) PHYS_MEMORY_START);
}
static inline unsigned lowlevel_mem_test_addr(void)
{
    return (unsigned)memTestAddressBus((datum *)PHYS_MEMORY_START,
				    PHYS_MEMORY_SIZE);
}
